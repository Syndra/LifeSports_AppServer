...
exports.getGymInfoBySports = function (request, response)
{
  ...
  request.on('end', () =>
  {
    ...
    console.log('Data : ', data);
    var connection = mysqlLoader.mysql_load();
    connection.query('SELECT distinct gym_ID, gym_name, gym_info, gym_location, avail_starttime, avail_endtime from gym natural join fac_info where subj_ID = ?',
...
exports.searchPrefGym = function (request, response)
{
  ...
  request.on('end', () =>
  {
    ...
    console.log('Data : ', data);
    var connection = mysqlLoader.mysql_load();
    connection.query('SELECT gym_ID, gym_name, gym_location from pref_gym_per_user natural join gym WHERE UDID = ?',
    [data.UDID],
    function(err, results){
      if(err)
        console.log(err);
