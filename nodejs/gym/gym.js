...
exports.getGymInfo = function (request, response)
{
  ...
  request.on('end', () =>
  {
    ...
    console.log('Data : ', data);
    var connection = mysqlLoader.mysql_load();
    connection.query('SELECT gym_ID, gym_name, gym_info, gym_location, avail_starttime, avail_endtime, gym_longitude, gym_latitude, isfavorite FROM gym natural join (SELECT ? as gym_ID, count(*) as isfavorite from pref_gym_per_user where gym_ID = ? and UDID = ?) as temp  WHERE gym_ID = ?',
...
exports.searchPrefGym = function (request, response)
{
  ...
  request.on('end', () =>
  {
    ...
    console.log('Data : ', data);
    var connection = mysqlLoader.mysql_load();
    connection.query('SELECT distinct gym_ID, gym_name, gym_location from pref_gym_per_user natural join gym natural join fac_info WHERE UDID = ? AND subj_ID = ?',
